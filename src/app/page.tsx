...
export default function Home() {
  ...
  const selectedCars = cars.filter(car => selected.includes(car.name));
  const total = selectedCars.reduce((sum, car) => sum + (car.price ? getDiscountedPrice(car.price) : 0), 0);

  return (
    <main className="max-w-3xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">เลือกรถที่คุณต้องการ</h1>
      {Object.entries(grouped).map(([category, list]) => (
        <div key={category} className="mb-6">
          <h2 className="text-xl font-semibold mb-2">🚗🚗🚗🚗 {category} 🚗🚗🚗🚗</h2>
          {list.map(car => (
            <label key={car.name} className="block mb-1">
              <input
                type="checkbox"
